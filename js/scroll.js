 ...
 *                                                                            *
 * Build up the relevant frame elements a keep a list of scroll callbacks.    *
 *                                                                            *
 * @summary Functions to handle the scroll effect.                            *
...
/** The initial and final path of a spade for wide screens. */
...
/** The initial and final path of a spade for mobile screens. */
...
/** The callbacks for every single frame for scrolling effects. */
const scrollCallbacks = [];

/**
 * Make sure the value lies between 0.0 and 1.0. Crop otherwise.
 *
 * @param value a floating point value.
 * @return cropped value that lies between 0.0 and 1.0.
 */
function crop(value) {
  if(value > 1.0) {
    return 1.0;
...
/**
 * Sets the correct spades path (wide vs. mobile) to all matching svg frame backgrounds.
 */
...
/**
 * Creates a function that calls a given callback for every given DOM element with the correct gauss value.
 *
 * @param w3cElement the DOM element that this callback resolves around.
 * @param callback The function that should be called the the correkt gauss value.
 */
function createGaussianCallback(w3cElement, callback) {
  ...
  /**
   * Calculate the elements position in window frame and calculates the gauss value.
   */
  return () => {
...
/**
 * Create a function for a specific DOM element that reacts to a change of the scroll position.
 *
 * @param frameDiv the DOM element this effect should be applied to.
 */
function createAnimationCallback(frameDiv) {
  ...
  /**
   * Apply the spades morph animation according to its gauss value.
   *
   * @param visibilityValue The gauss value that determines how much of the element is visible in the window frame.
   */
...
/**
 * Create the front page animations with anime.js.
 *
 * @return a promise that resolves once all animations are set up.
 */
...
/**
 * Calls all scroll callbacks for every frame on the page.
 *
...
/**
 * Does all preparations for the main content.
 * Call this after the circuit animation finished.
 *
 * @return a promise that resolves once everything is set up.
 */
